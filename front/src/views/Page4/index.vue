<template>
  <div>
    ...
    <div class="container">
      ...
      <div>
        <img style="width: 100%" src="@/assets/img/page4/tips.png">
      </div>
...
export default {
  ...
  // 使用 data 对象可以很方便地管理组件中所需的数据，并且能够保证组件的响应性能力
    return {
      ...
      showTableData1: [],
      showTableHeader1: [
          { code: "date", name: "姓名（文本格式）" },
          { code: "date", name:"平时总成绩（数字格式）" },
          { code: "date", name: "终结性成绩（数字格式）" },
          { code: "date",  name: "总评成绩（数字格式）" },
      ],
      showTableData2: [],
  ...
  computed:{
    showTableheader(){
      const maxVisibleHeaders = 4; // 控制前5列
      if (this.showTableHeader1.length <= maxVisibleHeaders){
        return this.showTableHeader1;
      } else {
  ...
  methods: {
    ...
    handlePreviewCharts() {
      ...
      const baseUrl = process.env.VUE_APP_BASE_URL
      const url = baseUrl + '/export_html?filename1='
...
.container { /*page1_bg.png图片文件放置在Vue项目的静态资源目录中，引入背景*/
  background-image: url("~@/assets/img/page4/bg.png");
