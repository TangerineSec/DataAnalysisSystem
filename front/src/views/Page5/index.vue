  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        ...
        <el-button type="success" icon="el-icon-upload" @click="handleAdd" style="margin: 0px 0px 0px 10px">上传Excel</el-button>
      ...
      <div style="margin: 50px auto; width: 200px"  v-show="showTableHeader1.length !== 0">前期表格</div>
      <el-table empty-text="导入数据后显示" :data="showTableData1" border class="table" ref="multipleTable" header-cell-class-name="table-header" :fit="true">
        <el-table-column v-for="(d, index) in showTableHeader1" :prop="d.code" :label="d.name"  align="center"></el-table-column>
            ...
            <div style="margin: 50px auto; width: 200px"  v-show="showTableHeader2.length !== 0">后期表格</div>
      <el-table empty-text="导入数据后显示" :data="showTableData2" border class="table" ref="multipleTable" header-cell-class-name="table-header" :fit="true">
        <el-table-column v-for="(d, index) in showTableHeader2" :prop="d.code" :label="d.name"  align="center"></el-table-column>
      </el-table>

      <div>
        <img style="width: 100%" src="@/assets/img/page5/tips.png">
      </div>
    ...
    <el-dialog title="点击方框或拖拽文件，可导入文件" v-model="diaLogShow"   width="30%">
      <el-form    >
        ...
        <el-form-item label="选择前期文件" prop="file">
          <el-upload
              class="upload-import"
              ref="upload1"
              action=""
         ...
         <el-form-item label="选择后期文件" prop="file">
          <el-upload
              class="upload-import"
              ref="upload2"
              action=""
...
export default {
  ...
  // 使用 data 对象可以很方便地管理组件中所需的数据，并且能够保证组件的响应性能力
    return {
      ...
      showTableHeader1: [
          { code: "date", name: "姓名" },
          { code: "date", name: "学号" },
          { code: "date", name:"所属行政班" },
          { code: "date", name: "所属学院" },
          { code: "date",  name: "课堂(40.0%)" },
          { code: "date", name: "考试(60.0%)" },
          { code: "date", name: "总成绩" },
      ],
      showTableData2: [],
      showTableHeader2: [
  ...
  computed:{
    showTableheader(){
      const maxVisibleHeaders = 7; // 控制前7列
      if (this.showTableHeader1.length <= maxVisibleHeaders){
        return this.showTableHeader1,this.showTableHeader2;
      } else {
        return this.showTableHeader1.slice(0,maxVisibleHeaders),this.showTableHeader2.slice(0,maxVisibleHeaders)
      }
    }
  },
  methods: {
    handleProcess(){
      ...
      axios.get(url).then(response => {
        ...
        this.showTableHeader2 = response.data.data.headers
        this.showTableData2 = response.data.data.data()
        this.filename =  ''
    ...
    handlePreviewCharts() {
      ...
      const baseUrl = process.env.VUE_APP_BASE_URL
      const url = baseUrl + '/export_rainclass?filename1='
          + this.filename1 + '&filename2='+this.filename2+'&sheet1=' + this.selectedSheet1 +'&sheet2='+this.selectedSheet2+ '&theme=' + this.selectedTheme
      window.open(url)
      return
      console.log('url :', url)
    ...
    handlePreview() {
      ...
      const baseUrl = process.env.VUE_APP_BASE_URL
      const url = baseUrl + '/get_two_excel_data?filename1='+this.filename1
          + '&filename2='+this.filename2+'&sheet1=' + this.selectedSheet1 +'&sheet2='+this.selectedSheet2
      console.log('url:', url)
      ...
      axios.get(url,).then(response => {
        ...
        this.showTableData1 =  response.data.data.data1.data.slice(0)
        this.showTableHeader2 =  response.data.data.data2.headers.slice(1)
        this.showTableData2 =  response.data.data.data2.data.slice(0)
          }
      )
    },
    submit() {
      //点击提交之后的函数处理
      const baseUrl = process.env.VUE_APP_BASE_URL
      const url = baseUrl + '/upload_two_file_and_get_sheet_name'
